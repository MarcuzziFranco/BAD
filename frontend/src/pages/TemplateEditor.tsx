...
export function TemplateEditor() {
  ...
  // Parsear JSON cuando cambia el contenido
  useEffect(() => {
    try {
  ...
  if (isEditing && isLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-muted-foreground">Cargando template...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="h-[calc(100vh-320px)]">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg">Vista de Árbol</CardTitle>
          </CardHeader>
              ...
              <TabsContent value="tree" className="h-[calc(100%-40px)] overflow-auto mt-2">
                {parsedJson ? (
                  ...
                  <p className="text-muted-foreground text-sm">
                    {parseError ? 'JSON inválido' : 'Escribe JSON válido para ver el árbol'}
                  </p>
                )}
              </TabsContent>
