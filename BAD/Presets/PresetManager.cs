...
using BAD.Presets.Mutations;
using Newtonsoft.Json.Linq;

namespace BAD.Presets;
...
public class PresetManager
{
    ...
    public void ApplyPreset(IPreset preset, JObject jsonTemplate)
    {
        // Limpiar configuraciones anteriores
        GeneratorJson.ClearDefaultValues();

        // Generar y aplicar nuevas configuraciones
        var configs = preset.GenerateConfig(jsonTemplate);
        
        foreach (var kvp in configs)
        {
            GeneratorJson.AddNewDefaultValue(kvp.Key, kvp.Value);
        }
    }
    ...
    /// </summary>
    public bool ApplyPresetByName(string name, JObject jsonTemplate)
    {
        var preset = GetPresetByName(name);
        if (preset == null) return false;

        ApplyPreset(preset, jsonTemplate);
        return true;
    }
    ...
    private string GetCategoryName(PresetCategory category) => category switch
    {
